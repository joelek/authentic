import * as mariadb from "mariadb";
import * as server from "../server";
import * as objects from "./objects";

let CONNECTION: mariadb.Connection | undefined;

export async function getConnection(): Promise<mariadb.Connection> {
	if (CONNECTION == null) {
		CONNECTION = await mariadb.createConnection({
			host: "localhost",
			port: 3306,
			user: "root",
			database: "authentic",
			password: "",
			bigIntAsNumber: true,
			decimalAsNumber: true,
			typeCast: (field, next) => {
				if ([mariadb.TypeNumbers.TINY, mariadb.TypeNumbers.SHORT, mariadb.TypeNumbers.LONG].includes(field.columnType) && field.columnLength === 1) {
					let value = field.int();
					return value == null ? null : value === 1;
				}
				return next();
			}
		});
		await CONNECTION.query(`SET SQL_MODE = 'ANSI_QUOTES,TRADITIONAL'`);
		/*
		await CONNECTION.query(`
			DROP TABLE IF EXISTS objects
		`, []);
		await CONNECTION.query(`
			CREATE TABLE IF NOT EXISTS objects (
				object_id VARCHAR(32) PRIMARY KEY NOT NULL,
					UNIQUE INDEX object_id (object_id),
				optional_boolean BOOL,
					UNIQUE INDEX optional_boolean (optional_boolean, object_id),
				optional_integer INT,
					UNIQUE INDEX optional_integer (optional_integer, object_id),
				optional_string VARCHAR(15),
					UNIQUE INDEX optional_string (optional_string, object_id)
			)
		`, []);
		for (let i = 0; i < 1000; i++) {
			await CONNECTION.query(`
				INSERT INTO objects (
					object_id,
					optional_boolean,
					optional_integer,
					optional_string
				) VALUES (
					?,
					?,
					?,
					?
				)
			`, [
				server.utils.generateHexId(32),
				Math.random() < 0.1 ? null : Math.random() < 0.5 ? true : false,
				Math.random() < 0.1 ? null : Math.floor(Math.random() * 10),
				Math.random() < 0.1 ? null : String.fromCharCode("A".charCodeAt(0) + Math.floor(Math.random() * 10))
			]);
		}
		*/
	}
	return CONNECTION;
};

const DETAIL: server.stores.store.DatabaseObjectStoreDetail = {
	getConnection: getConnection,
	generateId: () => server.utils.generateHexId(32)
};

export const OBJECTS = new server.stores.store.DatabaseObjectStore(DETAIL, "objects", "object_id", objects.Object, {});
