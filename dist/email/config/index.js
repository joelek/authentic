"use strict";
// This file was auto-generated by @joelek/autoguard. Edit at own risk.
Object.defineProperty(exports, "__esModule", { value: true });
exports.Autoguard = exports.MailerOptions = exports.Attachment = void 0;
const autoguard = require("@joelek/autoguard/dist/lib-shared");
exports.Attachment = autoguard.guards.Object.of({
    "filename": autoguard.guards.String,
    "mime": autoguard.guards.String,
    "data": autoguard.guards.Binary
}, {});
exports.MailerOptions = autoguard.guards.Object.of({
    "smtp": autoguard.guards.Object.of({
        "hostname": autoguard.guards.String,
        "port": autoguard.guards.Number,
        "username": autoguard.guards.String,
        "password": autoguard.guards.String
    }, {})
}, {
    "defaults": autoguard.guards.Object.of({}, {
        "to_address": autoguard.guards.String,
        "to_name": autoguard.guards.String,
        "from_address": autoguard.guards.String,
        "from_name": autoguard.guards.String,
        "reply_address": autoguard.guards.String,
        "reply_name": autoguard.guards.String
    })
});
var Autoguard;
(function (Autoguard) {
    Autoguard.Guards = {
        "Attachment": autoguard.guards.Reference.of(() => exports.Attachment),
        "MailerOptions": autoguard.guards.Reference.of(() => exports.MailerOptions)
    };
    Autoguard.Requests = {};
    Autoguard.Responses = {};
})(Autoguard = exports.Autoguard || (exports.Autoguard = {}));
;
